...
double D_GGX(double NoH, double roughness){
    ...
    double tmp = alpha / std::max(1e-8, (NoH * NoH * (alpha * alpha - 1.0) + 1.0) );
    return tmp * tmp / PI;
}
...
double G_Smith(double NoV, double NoL, double roughness){
    double k = std::max(1e-8, roughness * roughness / 2.0);
    //double k = std::max(1e-8, (roughness + 1) * (roughness + 1) / 8.0);
...
Eigen::Vector3d disney(Eigen::Vector3d l, Eigen::Vector3d v, 
    ...
    double NoH = std::max(n.dot(h), 1e-8);
    double NoL = std::max(n.dot(l), 1e-8);
    double NoV = std::max(n.dot(v), 1e-8);
    double VoH = std::max(v.dot(h), 1e-8);
